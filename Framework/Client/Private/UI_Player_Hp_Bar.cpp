...
CUI_Player_Hp_Bar::CUI_Player_Hp_Bar(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
...
void CUI_Player_Hp_Bar::Tick(_float fTimeDelta)
{
	...
	m_fSizeX = max(0, 176.f * (m_pPlayerParam->fCurrentHp) / (m_pPlayerParam->fMaxHp));
	m_pTransformCom->Set_Scaled(_float3(m_fSizeX, m_fSizeY, 1.f));
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(m_fX + m_fSizeX / 2 - g_iWinSizeX * 0.5, -m_fY - m_fSizeY / 2 + g_iWinSizeY * 0.5, 0.f, 1.f));
}
...
HRESULT CUI_Player_Hp_Bar::Add_Component()
{
...
HRESULT CUI_Player_Hp_Bar::Bind_ShaderResources()
{
...
CUI_Player_Hp_Bar * CUI_Player_Hp_Bar::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	/* 원형객체를 생성한다. */
	CUI_Player_Hp_Bar* pInstance = new CUI_Player_Hp_Bar(pDevice, pContext);
