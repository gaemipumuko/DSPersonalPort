...
CFireLinkShrine_Pick::CFireLinkShrine_Pick(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

CFireLinkShrine_Pick::CFireLinkShrine_Pick(const CFireLinkShrine_Pick& rhs)
	: CGameObject(rhs)
{
}

HRESULT CFireLinkShrine_Pick::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CFireLinkShrine_Pick::Initialize(void* pArg)
{
...
void CFireLinkShrine_Pick::Tick(_float fTimeDelta)
{
...
void CFireLinkShrine_Pick::LateTick(_float fTimeDelta)
{
	//m_pRendererCom->Add_RenderGroup(CRenderer::RG_NONBLEND, this);
}

HRESULT CFireLinkShrine_Pick::Render()
{
...
HRESULT CFireLinkShrine_Pick::Add_Component()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Components(m_pGameInstance->Get_NextLevelIndex(), TEXT("Prototype_Component_Model_Fire_Link_Shrine_Pick"),
...
HRESULT CFireLinkShrine_Pick::Bind_ShaderResources()
{
...
CFireLinkShrine_Pick* CFireLinkShrine_Pick::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	/* 원형객체를 생성한다. */
	CFireLinkShrine_Pick* pInstance = new CFireLinkShrine_Pick(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CFireLinkShrine_Pick");
...
CGameObject* CFireLinkShrine_Pick::Clone(void* pArg)
{
	/* 실사용객체를 생성한다. 복사생성자호출. */
	CFireLinkShrine_Pick* pInstance = new CFireLinkShrine_Pick(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CFireLinkShrine_Pick");
		Safe_Release(pInstance);
	}
...
void CFireLinkShrine_Pick::Free()
{
	__super::Free();
