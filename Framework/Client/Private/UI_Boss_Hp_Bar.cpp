...
void CBoss_Hp_Bar::Update(_float fHp)
{
	m_fSizeX = 660.f *fHp;
	m_pTransformCom->Set_Scaled(_float3(max(m_fSizeX,0.f), m_fSizeY, 1.f));
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(m_fX + m_fSizeX / 2 - g_iWinSizeX * 0.5, -m_fY - m_fSizeY / 2 + g_iWinSizeY * 0.5, 0.f, 1.f));

}

HRESULT CBoss_Hp_Bar::Add_Component()
{
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Components(m_pGameInstance->Get_NextLevelIndex(), TEXT("Prototype_Component_Texture_Boss_Hp_Bar"),
...
HRESULT CBoss_Hp_Bar::Bind_ShaderResources()
{
...
CBoss_Hp_Bar * CBoss_Hp_Bar::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, void* pArg)
{
	/* 원형객체를 생성한다. */
	...
	{
		MSG_BOX("Failed to Created : CBoss_Hp_Bar");
...
{	
	...
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CBoss_Hp_Bar");
		Safe_Release(pInstance);
	}
