...
HRESULT CInteraction::Initialize(void* pArg)
{
	...
	lstrcpy(m_szText, strTextBox.c_str());
...
void CInteraction::Tick(_float fTimeDelta)
{

}

void CInteraction::LateTick(_float fTimeDelta)
{
...
#pragma region 텍스트 출력
	...
	m_pGameInstance->Render_Text(TEXT("Font_Default"), wstring(m_szText), _float2(635.f, 613.f), 0.45f, Colors::White);
...
HRESULT CInteraction::Add_Component()
{
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Components(m_pGameInstance->Get_NextLevelIndex(), TEXT("Prototype_Component_Texture_Interaction"),
...
CInteraction * CInteraction::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext , void* pArg)
{
	/* 원형객체를 생성한다. */
