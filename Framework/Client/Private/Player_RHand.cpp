#include "stdafx.h"
#include "..\Public\Player_RHand.h"

#include "GameInstance.h"

CPlayer_RHand::CPlayer_RHand(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

CPlayer_RHand::CPlayer_RHand(const CPlayer_RHand & rhs)	
	: CGameObject(rhs)
{
}

HRESULT CPlayer_RHand::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CPlayer_RHand::Initialize(void* pArg)
{
...
void CPlayer_RHand::Tick(_float fTimeDelta)
{


}

void CPlayer_RHand::LateTick(_float fTimeDelta)
{
...
HRESULT CPlayer_RHand::Render_Shadow()
{
...
HRESULT CPlayer_RHand::Add_Component()
{
	/* For.Com_Renderer */
	...
	//BoundingDesc.vExtents = _float3(2.f, 2.f, 20.f);
	//_vector vPosition = m_pTransformCom->Get_State(CTransform::STATE_POSITION);
	//BoundingDesc.vCenter = _float3(vPosition.m128_f32[0], vPosition.m128_f32[1], vPosition.m128_f32[2]);
	BoundingDesc.vCenter = _float3(0.f, 0.f, 40.f);
	BoundingDesc.fRadius = 20.f;
	//BoundingDesc.vRadians = _float3(0.f, XMConvertToRadians(20.f), 0.f);

	if (FAILED(__super::Add_Components(m_pGameInstance->Get_NextLevelIndex(), TEXT("Prototype_Component_Collider_SPHERE"),
...
HRESULT CPlayer_RHand::Bind_ShaderResources()
{
...
CPlayer_RHand * CPlayer_RHand::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	/* 원형객체를 생성한다. */
	CPlayer_RHand* pInstance = new CPlayer_RHand(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CPlayer_RHand");
...
CGameObject* CPlayer_RHand::Clone(void* pArg)
{	
	/* 실사용객체를 생성한다. 복사생성자호출. */
	CPlayer_RHand* pInstance = new CPlayer_RHand(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CPlayer_RHand");
		Safe_Release(pInstance);
	}
