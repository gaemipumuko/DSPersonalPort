...
class CYhorm final : public CGameObject
{
	enum YHORM_STATE { STATE_IDLE, STATE_ALERT, STATE_ATTACK, STATE_END };

private:
	CYhorm(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CYhorm(const CYhorm& rhs);
	virtual ~CYhorm() = default;
...
private:
	...
	_uint		m_iPrevAnimIndex = {40};
	_uint		m_iCurrentAnimIndex = {40};
	_uint		m_iNextAnimIndex = {40};
...
public:
	/* 원형객체를 생성할 때 */
	static CYhorm* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
